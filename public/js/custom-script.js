(function ($) {
  ...
  //MixitUp Gallery Filters + Gallery Load More (pehle 9, phir 3-3)
  // Filter: MixItUp ke controls band, manual click binding se filter trigger karte hain
  if ($(".filter-list").length) {
    var mixOptions = {
      controls: { enable: false }
    };

    if ($(".mixitup-gallery").length && $("#gallery-load-more-btn").length) {
      var galleryVisibleCount = 9;
      var GALLERY_INITIAL = 9;
      var GALLERY_STEP = 3;
      var galleryAnimateReveal = false;

      function applyGalleryLoadMore() {
        var $wrap = $(".mixitup-gallery").first();
        var $list = $wrap.find(".filter-list").first();
        var currentFilter = $wrap.find(".filter.active").attr("data-filter") || "all";
        var $items =
          currentFilter === "all"
            ? $list.find(".gallery-item.mix")
            : $list.find(".gallery-item.mix" + currentFilter);

        $items.each(function (i) {
          var hide = i >= galleryVisibleCount;
          var $el = $(this);
          $el.toggleClass("gallery-load-hidden", hide);
          if (!hide) $el.css("display", "inline-block");
          $el.removeClass("gallery-load-reveal");
        });

        if (galleryAnimateReveal) {
          var startIdx = Math.max(0, galleryVisibleCount - GALLERY_STEP);
          $items.slice(startIdx, galleryVisibleCount).each(function () {
            var $el = $(this);
            $el.addClass("gallery-load-reveal");
            setTimeout(function () {
              $el.removeClass("gallery-load-reveal");
            }, 520);
          });
          galleryAnimateReveal = false;
        }

        var total = $items.length;
        $("#gallery-showing-count").text(Math.min(galleryVisibleCount, total));
        $("#gallery-total-count").text(total);

        var $moreWrap = $(".gallery-load-more-wrap");
        if (galleryVisibleCount >= total || total === 0) {
          $moreWrap.addClass("hide-load-more");
        } else {
          $moreWrap.removeClass("hide-load-more");
        }
      }

      mixOptions.callbacks = {
        onMixLoad: function () {
          setTimeout(applyGalleryLoadMore, 150);
        },
        onMixEnd: function () {
          setTimeout(applyGalleryLoadMore, 150);
        }
      ...
      // Filter tabs: jQuery se show/hide (MixItUp par depend nahi)
      $(".gallery-section .mixitup-gallery").each(function () {
        var $gallery = $(this);
        var $list = $gallery.find(".filter-list").first();
        $gallery.off("click.portfolioFilter", ".filter").on("click.portfolioFilter", ".filter", function (e) {
          e.preventDefault();
          var $btn = $(this);
          var filterVal = $btn.attr("data-filter");
          if (!filterVal) return;
          $gallery.find(".filter").removeClass("active");
          $btn.addClass("active");
          var filterClass = filterVal === "all" ? null : filterVal.replace(/^\./, "");
          $list.find(".gallery-item.mix").each(function () {
            var $el = $(this);
            var show = filterClass === null || $el.hasClass(filterClass);
            $el.removeClass("gallery-filter-reveal");
            $el.css("display", show ? "inline-block" : "none");
            if (show) $el.addClass("gallery-filter-reveal");
          });
          setTimeout(function () {
            $list.find(".gallery-item.mix").removeClass("gallery-filter-reveal");
          }, 520);
          galleryVisibleCount = GALLERY_INITIAL;
          applyGalleryLoadMore();
        });
      });
      ...
      $("#gallery-load-more-btn").on("click", function (e) {
        ...
        galleryAnimateReveal = true;
        applyGalleryLoadMore();
      });
    }

    $(".filter-list").mixItUp(mixOptions);

    // Gallery section jahan Load More nahi hai: wahan bhi filter jQuery se
    $(".gallery-section .mixitup-gallery").each(function () {
      var $gallery = $(this);
      if ($gallery.find("#gallery-load-more-btn").length) return;
      var $list = $gallery.find(".filter-list").first();
      $gallery.off("click.portfolioFilter", ".filter").on("click.portfolioFilter", ".filter", function (e) {
        e.preventDefault();
        var $btn = $(this);
